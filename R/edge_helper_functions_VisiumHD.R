...
#'
#' @param r A \code{\link[terra]{SpatRaster}} object with binary values (1=outlier, 0=normal).
#' @param min_cluster_size Minimum size (in bins) for small hole removal.
#'
#' @return A \code{\link[terra]{SpatRaster}} object after applying focal
...
focal_transformations_terra <- function(r, min_cluster_size = 5) {
  if (!inherits(r, "SpatRaster")) {
    stop(
      "This function requires a terra::SpatRaster object.\n",
...
problemAreas_WithMorphology_terra <- function(.xyz,
  ...
  if (resolution == "16um") {
    # 16um has 4* fewer bins than 8um (2* in each dimension)
    # So reduce min_cluster_size accordingly
    adj_min_size <- max(2, min_cluster_size %/% 4)
    message(sprintf(
      "    [16um Mode] Adjusted min_cluster_size: %d -> %d",
  ...
  message(sprintf(
    "    Kept %d clusters (>=%d bins): %d bins total",
  ...
  if (n_filtered > 0) {
    ...
    message(sprintf(
      "    Filtered %d bins in %d small clusters",
