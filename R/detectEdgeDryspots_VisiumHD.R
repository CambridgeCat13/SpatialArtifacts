...
#' artifact detection columns added to \code{colData}:
#' \itemize{
#'   \item \code{[name]_edge}: Logical, indicates if a bin is an outlier within the buffer zone.
#'   \item \code{[name]_problem_id}: Character, unique ID for each detected interior problem area.
#'   \item \code{[name]_problem_size}: Numeric, the number of bins within each problem area cluster.
#' }
#'
#' @details
...
detectEdgeArtifacts_VisiumHD <- function(
  ...
  samples = "sample_id",
  mad_threshold = 3,
  buffer_width_um = 80, # Physical distance in um (default: ~10 bins at 8um)
  min_cluster_area_um2 = 1280, # Physical area in um^2 (default: ~5 bins at 16um)
  batch_var = "sample_id",
  ...
  if (!requireNamespace("terra", quietly = TRUE)) {
    stop(
      "Package 'terra' required for VisiumHD.\n",
  ...
  if (qc_metric == "sum_gene" && !"sum_gene" %in% colnames(colData(spe))) {
    ...
    if ("detected" %in% colnames(colData(spe))) {
      ...
      stop(
        "QC metric 'sum_gene' not found. Available columns: ",
  ...
  if (verbose) {
    ...
    message("================================================================")
    message(sprintf(
      "Resolution: %s (bin size = %d*%d um)",
      resolution, bin_size_um, bin_size_um
    ))
    message(sprintf(
      "Coordinate Range: X[%d-%d], Y[%d-%d] (bin indices)",
      coord_min_x, coord_max_x, coord_min_y, coord_max_y
    ))
    message(sprintf(
      "Buffer Width: %d um -> %d bins",
      buffer_width_um, buffer_width_bins
    ))
    message(sprintf(
      "Min Cluster Area: %d um2 -> %d bins",
  ...
  problem_areas_list <- lapply(sampleList, function(sample_name) {
    ...
    if (verbose) {
      ...
      message(sprintf(
        "  %s: %d interior bins in %d clusters (%d edge filtered)",
